...
import { hsl2rgb, hex2rgb, rgb2hsl, rgb2hex, hex2xyz, hex2contrast, hex2lab, lab2rgb, hex2hsl, hsl2hex } from '@csstools/convert-colors';
...
var _wksDefine$1 = function (name) {
  ...
  if (name.charAt(0) != '_' && !(name in $Symbol)) defineProperty$3($Symbol, name, { value: _wksExt$1.f(name) });
};
...
var f$e = _descriptors$1 ? gOPD$2 : function getOwnPropertyDescriptor(O, P) {
  O = _toIobject$1(O);
  P = _toPrimitive$1(P, true);
  if (_ie8DomDefine$1) try {
    return gOPD$2(O, P);
...
var ObjectProto$2 = Object[PROTOTYPE$5];
...
if (!USE_NATIVE$2) {
  $Symbol$1 = function Symbol() {
    ...
    if (_descriptors$1 && setter$1) setSymbolDesc$1(ObjectProto$2, tag, { configurable: true, set: $set });
    return wrap$1(tag);
  };
...
_export$1(_export$1.P + _export$1.F * (_fails$1(function () {
  ...
  sort: function sort(comparefn) {
    return comparefn === undefined
      ? $sort.call(_toObject$1(this))
      : $sort.call(_toObject$1(this), _aFunction$1(comparefn));
  }
});
...
function () {
  ...
  _createClass(Color, [{
    ...
    key: "hsl",
    get: function get() {
      var rgb = this._rgb.map(function (c) {
        return c / 255 * 100;
      });
...
function getSortedPaletteKeys(palette) {
  var ids = [];

  for (var sw in palette) {
...
function findBackgroundColor(palette) {
  var ids = getSortedPaletteKeys(palette);
  ...
  if (!highestNoneWhite || palette[ids[0]].getPopulation() > 2.5 * palette[highestNoneWhite].getPopulation()) {
    return palette[ids[0]].getHex();
  }
...
function isBlackOrWhite(hsl) {
  return isWhite(hsl) || isBlack(hsl);
}
function selectForegroundColor(moreVibrant, vibrant, moreMuted, muted, dominant, fallback, mapSize) {
  var candidate = selectVibrantCandidate(moreVibrant, vibrant, mapSize);
  ...
  if (candidate) {
    ...
    } else if (candidate.getPopulation() < dominant.getPopulation() * 0.01 && dominant.getHsl()[1] > 0.19) {
...
function findForegroundColor(backgroundColor, palette, mapSize) {
  var ids = getSortedPaletteKeys(palette);

  if (isColorLight(backgroundColor)) {
    return selectForegroundColor(palette.DarkVibrant, palette.Vibrant, palette.DarkMuted, palette.Muted, palette[ids[0]], '#000', mapSize);
  } else {
    return selectForegroundColor(palette.LightVibrant, palette.Vibrant, palette.LightMuted, palette.Muted, palette[ids[0]], '#fff', mapSize);
  }
}
function isColorLight(color) {
  var xyz = hex2xyz(color);
  return xyz[1] > 50;
}
function selectVibrantCandidate(first, second, mapSize) {
  var firstValid = hasEnoughPopulation(first, mapSize);
  var secondValid = hasEnoughPopulation(second, mapSize);
...
function selectMutedCandidate(first, second, mapSize) {
  var firstValid = hasEnoughPopulation(first, mapSize);
  var secondValid = hasEnoughPopulation(second, mapSize);

  if (firstValid && secondValid) {
    var firstSaturation = first.getHsl()[1];
    var secondSaturation = second.getHsl()[1];
    var populationFraction = first.getPopulation() / second.getPopulation();
...
function hasEnoughPopulation(color, mapSize) {
  return color && color.getPopulation() > mapSize * 0.002;
}
function ensureColors(backgroundColor, foregroundColor) {
  var backY = hex2xyz(backgroundColor)[1];
  var foreY = hex2xyz(foregroundColor)[1];
  var contrast = hex2contrast(backgroundColor, foregroundColor);
  var backgroundLight = backY > foreY && hex2contrast(backgroundColor, '#000') > 4.5 || backY < foreY && hex2contrast(backgroundColor, '#fff') <= 4.5;
  ...
  if (contrast > 4.5) {
    ...
    if (backgroundLight) {
      var lab = hex2lab(primary);
      lab[0] = lab[0] + 20 <= 100 ? lab[0] + 20 : 100;
      secondary = lab2hex(lab[0], lab[1], lab[2]);
    } else {
      var _lab = hex2lab(primary);

      _lab[0] = _lab[0] - 10 >= 0 ? _lab[0] - 10 : 0;
      secondary = lab2hex(_lab[0], _lab[1], _lab[2]);
    }

    if (hex2contrast(backgroundColor, secondary) > 4.5) {
      ok = true;
    }
  }

  if (!ok) {
    secondary = backgroundLight ? findContrastColor(foregroundColor, backgroundColor) : findContrastColorAgainstDark(foregroundColor, backgroundColor);

    if (backgroundLight) {
      var _lab2 = hex2lab(secondary);

      _lab2[0] = _lab2[0] - 20 > 0 ? _lab2[0] - 20 : 0;
      primary = lab2hex(_lab2[0], _lab2[1], _lab2[2]);
    } else {
      var _lab3 = hex2lab(secondary);

      _lab3[0] = _lab3[0] + 10 < 100 ? _lab3[0] + 10 : 100;
      primary = lab2hex(_lab3[0], _lab3[1], _lab3[2]);
    }
  }
...
function findContrastColor(foregroundColor, backgroundColor) {
  var lab = hex2lab(foregroundColor);
  var low = 0;
  var high = lab[0];
  var color = foregroundColor;

  for (var i = 0; i < 15 && high - low > 0.00001; i++) {
    var l = (low + high) / 2;
    lab[0] = l;
    color = lab2hex(lab[0], lab[1], lab[2]);

    if (hex2contrast(color, backgroundColor) > 4.5) {
      low = l;
    } else {
      high = l;
    }
...
function findContrastColorAgainstDark(foregroundColor, backgroundColor) {
  var hsl = hex2hsl(foregroundColor);
  var low = hsl[2];
  var high = 100;
  var color = foregroundColor;
  ...
  for (var i = 0; i < 15 && high - low > 0.00001; i++) {
    var l = (low + high) / 2;
    hsl[2] = l;
    color = hsl2hex(hsl[0], hsl[1], hsl[2]);
...
var MediaStylePalette =
/*#__PURE__*/
function () {
  function MediaStylePalette(option) {
    _classCallCheck(this, MediaStylePalette);
  ...
  _createClass(MediaStylePalette, [{
    key: "use",
    value: function use(option) {
      if (option !== null && option !== undefined && _typeof(option) !== 'object') {
    ...
    key: "getPalette",
    value: function () {
      var _getPalette = _asyncToGenerator(
      /*#__PURE__*/
      regenerator.mark(function _callee() {
        var x, y, url, palette, backgroundColor, foregroundColor;
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                return _context.abrupt("return", this.palette);

              case 2:
                x = Math.cos(this.directionRad);
                y = -Math.sin(this.directionRad);
                this.ctx.drawImage(this.ref, x < 0 ? x * this.option.maxDimension : 0, y < 0 ? y * this.option.maxDimension : 0, (1 + Math.abs(x)) * this.option.maxDimension, (1 + Math.abs(y)) * this.option.maxDimension);
                url = this.canvas.toDataURL();
                _context.next = 8;
                return from(url).maxDimension(this.option.maxDimension).clearFilters().getPalette();

              case 8:
                palette = _context.sent;
                backgroundColor = findBackgroundColor(palette);
                _context.next = 12;
                return from(url).addFilter(function (r, g, b) {
                  var color = Color.fromRgb(r, g, b);
                  var bgColor = Color.fromHex(backgroundColor);
                  var diff = Math.abs(color.hsl[0] - bgColor.hsl[0]);
                  return diff > 10 && diff < 350;
                }).addFilter(function (r, g, b) {
                  var color = Color.fromRgb(r, g, b);
                  return !isBlackOrWhite(color.hsl);
                }).maxDimension(this.option.maxDimension).getPalette();

              case 12:
                palette = _context.sent;
                foregroundColor = findForegroundColor(backgroundColor, palette, this.option.maxDimension * this.option.maxDimension);
                return _context.abrupt("return", ensureColors(backgroundColor, foregroundColor));
      ...
      function getPalette() {
        return _getPalette.apply(this, arguments);
      }
...
export { Color, MediaStylePalette };
//# sourceMappingURL=media-style-palette.esm.js.map
